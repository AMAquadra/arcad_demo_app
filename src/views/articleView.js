...
export default function ArticleView(props) {
  ...
  async function addArticle() {
    ...
    const isSuccess = await createArticle(nArticle);
    if (isSuccess) {
              ...
              <div style={{ overflow: "hidden" }} >
                {transitionDescription((style, item) => item && (
                  <animated.div style={style}>
                    <TextAreaField
                      for="description" label="Edit description" value={description.DESCRIPTION}
                      tooltip={<>A description of the article. <br></br> Can't be empty</>}
                      onChange={(e) => {
                        setDescription(prevState => ({ ...prevState, DESCRIPTION: e.target.value }))
                      }}
                    >
                    </TextAreaField>
                  </animated.div>
